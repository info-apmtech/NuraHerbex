...
<html>
  <body style=""font-family: Arial, Helvetica, sans-serif; font-size: 14px; color: #222;"">
    ...
    <p style=""font-size:12px;color:#666;"">You’re receiving this email because you subscribed at <strong>nuraherbex.com</strong>.</p>
		...
		{
			// Make sure you're checking the correct product table:
			var product = await _db.ProductDetails.FirstOrDefaultAsync(p => p.Id == item.ProductId);
			if (product == null)
				return IdentityResult.Failed(new IdentityError { Description = "Product does not exist." });
			...
			if (existing == null)
			{
				await _db.CartItems.AddAsync(new CartItem
				{
					...
					Quantity  = item.Quantity > 0 ? item.Quantity : 1, // first add
					Price     = product.Amount                           // snapshot unit price
				});
			}
			//else
			//{
			//	existing.Quantity += (item.Quantity > 0 ? item.Quantity : 1); // <-- increment on every click
			//																  // Optional: existing.Price = product.Amount; // if you want latest price policy
